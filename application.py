...
-------------------------------------------------------------------------------
Description :
Text Editor is a python tkinter based text editor lashed with many of the
...
class TextEditor(tk.Tk):
	...
	def draw_menu(self):
		...
		# help_menu
		help_menu.add_command(label='Help on Text Editor', compound=tk.LEFT, image=self.help_icon, command=get_help)
		help_menu.add_command(label='Shortcut Keys', compound=tk.LEFT, image=self.shortcut_icon, command=show_shortcuts)
		help_menu.add_command(label='About Text Editor', compound=tk.LEFT, image=self.coastline_icon, command=show_about)
	...
	def change_wm_title(self, event=None):	
		...
		self.title(self.filename + ' - Text Editor')
	...
	def save_file(self, event=None):
		...
		if content:
			...
			messagebox.showerror('Text Editor', 'Empty content')
	...
	def find(self, event=None):
		text = tkinter.simpledialog.askstring('Text Editor', 'Enter text to find')
		if text:
	...
	def find_regex(self, event=None):
		regex = tkinter.simpledialog.askstring('Text Editor', 'Enter regular expression\nto find')
		if regex:
			...
			self.submit.configure(image=self.exit_icon)
			self.submit.configure(command=self.remove_find_tag)
			messagebox.showinfo('Text Editor', f'{len(lst)} matches found')
	...
	def find_sum(self):
		...
		if nums:
			...
			messagebox.showinfo('Text Editor', f'Sum of {len(nums)} numbers\nfound is {sum_total}')
		else:
			messagebox.showinfo('Text Editor', 'No number found')
	...
	def find_average(self):
		...
		if nums:
			...
			messagebox.showinfo('Text Editor', f'Average of {len(nums)} numbers\nfound is {avg}')
		else:
			messagebox.showinfo('Text Editor', 'No number found')

	def count_occurence(self):
		term = tkinter.simpledialog.askstring('Text Editor', 'Enter term to count\nits occurence')
		if term:
			regex = re.compile(term)
			string = self.textbox.get('1.0', 'end-1c')
			nums = re.findall(regex, string)
			if nums:
				messagebox.showinfo('Text Editor', f'{term} occurs {len(nums)} times in the string')
			else:
				messagebox.showinfo('Text Editor', 'No occurence found')
...
def show_about():
	...
	label = tk.Label(win, text='Text Editor - A modern day text Editor', fg='red',
				font=('Calibri', 12))
	label.grid(row=0, column=0)
	text = tk.Text(win, width=30, height=4)
	text.insert(tk.END, 'Author : Tiwari Prabhat\n')
...
def show_shortcuts():
	...
	header = tk.Label(win, text='Shortcut keys for Text Editor')
